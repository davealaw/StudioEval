...
exit = sys.exit  # allow tests to patch the CLI exit path

try:  # Support both package (relative) and direct script execution
    from .__version__ import __version__
    from .config.comm_config import load_comm_config
    from .utils.logging import setup_logging
except ImportError:  # pragma: no cover - fallback when run as standalone script
    from __version__ import __version__  # type: ignore
    from config.comm_config import load_comm_config  # type: ignore
...
def main():
    ...
    parser.add_argument(
        "--version", action="version", version=f"StudioEval {__version__}"
    )
    parser.add_argument("--model", help="Test a specific model ID")
    parser.add_argument(
        "--model-filter", help="Glob-style filter for models, e.g. 'qwen3*' or '*4B*'"
    )
    parser.add_argument(
        "--model-set", type=str, help="Path to a file listing model IDs to evaluate"
    )
    parser.add_argument(
        "--model-arch",
        dest="model_arch",
        default=None,
        help="Glob-style architecture filter of models, e.g. 'qwen3' or 'qwen*'",
    )
    parser.add_argument("--all", action="store_true", help="Test all loaded models")
    parser.add_argument(
        "--datasets-config", type=str, help="Path to datasets_to_run JSON file"
    )
    parser.add_argument(
        "--sample-size",
        type=int,
        default=0,
        help="Number of samples to evaluate from each dataset (0 for all)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log-file",
        default=None,
        help="Optional file to write logs instead of console",
    )
    parser.add_argument(
        "--comm-config", type=str, help="Path to communication config JSON"
    )
    parser.add_argument(
        "--raw-duration",
        action="store_true",
        help="Output duration in seconds instead of human-readable format",
    )
