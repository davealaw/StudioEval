...
class TestDatasetRegistryIntegration:
    ...
    def test_registry_evaluation_dispatch(self, dataset_pipeline_client):
        ...
        # Test evaluation dispatch - this will hit real server but complete without hanging
...
class TestModelDatasetPipelineIntegration:
    ...
    def test_model_switching_between_datasets(self, mock_load, mock_query_model, dataset_pipeline_client):
        ...
        datasets = [
            {"eval_type": "grammar", "dataset_path": "grammar.jsonl", "dataset_name": "grammar_test"},
            {"eval_type": "gsm8k", "dataset_path": "math.jsonl", "dataset_name": "math_test"}
        ]
        
        with patch('eval_datasets.custom.grammar.evaluate_grammar_dataset') as mock_grammar, \
             patch('eval_datasets.huggingface.gsm8k.evaluate_gsm8k_dataset') as mock_math:
    ...
    @patch('models.model_handling.query_model')
    @patch('utils.data_loading.load_json_dataset_with_config')
    def test_multiple_models_same_dataset(self, mock_load, mock_query_model, dataset_pipeline_client):
...
class TestEvaluationResultPipelineIntegration:
    ...
    def test_result_aggregation_pipeline(self, mock_load, mock_query, dataset_pipeline_client):
...
class TestPipelineConfigurationIntegration:
    ...
    def test_invalid_config_pipeline_integration(self, dataset_pipeline_client, tmp_path):
