...
class TestEnhancedEvaluationFlow:
    ...
    def test_single_model_grammar_evaluation(
        self, mock_load_dataset, mock_query_model, enhanced_mock_client
    ):
        ...
        with (
            patch.object(DatasetRegistry, "evaluate_dataset") as mock_registry_eval,
            patch("builtins.open"),
        ):
    ...
    def test_multiple_models_evaluation(self, enhanced_mock_client):
        ...
        # Mock the registry's evaluate_dataset method to return different
        # results per call.
        with (
    ...
    @patch("core.evaluator.default_datasets_to_run", [])
    def test_multiple_datasets_evaluation(
        self, mock_load_dataset, mock_query_model, enhanced_mock_client
    ):
    ...
    def test_server_down_handling(self, enhanced_mock_client):
        ...
        result = orchestrator.run_evaluation(model="qwen-7b", datasets_config=[])
    ...
    def test_invalid_model_handling(self, enhanced_mock_client):
        ...
        with pytest.raises(ValueError, match="Model nonexistent-model not available"):
            orchestrator.run_evaluation(model="nonexistent-model", datasets_config=[])
...
class TestErrorRecoveryScenarios:
        ...
        with (
            ...
            result = orchestrator.run_evaluation(
                ...
                datasets_config=[
                    {
                        "eval_type": "grammar",
                        "dataset_path": "working.json",
                        "dataset_name": "working_test",
                    },
                    {
                        "eval_type": "grammar",
                        "dataset_path": "failing.json",
                        "dataset_name": "failing_test",
                    },
                ],
            )
    ...
    def test_model_loading_failure_recovery(self, enhanced_mock_client):
        ...
        with pytest.raises(Exception, match="Failed to load model"):
            orchestrator.run_evaluation(model="failing-model", datasets_config=[])
