...
def mock_model_client():
    ...
    return MockModelClient(
        responses=responses,
        available_models=["test-model", "grammar-model", "math-model"],
    )
...
def orchestrator(model_manager, dataset_registry):
    ...
    return EvaluationOrchestrator(
        model_manager=model_manager, dataset_registry=dataset_registry
    )
...
class TestEvaluationOrchestrator:
    ...
    @patch(
        "core.evaluator.default_datasets_to_run", []
    )  # Use empty list to avoid actual evaluations
