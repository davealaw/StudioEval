...
class TestCLIArgumentParsing:
    ...
    def test_basic_model_argument(self):
        ...
        with (
            patch("sys.argv", ["studioeval.py", *test_args]),
            patch("core.evaluator.EvaluationOrchestrator") as mock_orchestrator,
    ...
    def test_logging_arguments(self):
        ...
        with (
            patch("sys.argv", ["studioeval.py", *test_args]),
            patch("core.evaluator.EvaluationOrchestrator") as mock_orchestrator,
            patch.object(cli_module, "setup_logging") as mock_logging,
        ):
            mock_instance = Mock()
            mock_instance.run_evaluation.return_value = True
            mock_orchestrator.return_value = mock_instance
    ...
    def test_communication_config_argument(self):
        ...
        with (
            ...
            patch("core.evaluator.EvaluationOrchestrator") as mock_orchestrator,
            patch.object(cli_module, "setup_logging"),
            patch.object(cli_module, "load_comm_config") as mock_load_config,
        ):
            mock_instance = Mock()
            mock_instance.run_evaluation.return_value = True
            mock_orchestrator.return_value = mock_instance
...
class TestCLIErrorHandling:
    ...
    def test_evaluation_failure_exits_with_code_1(self):
        ...
        with (
            ...
            patch("core.evaluator.EvaluationOrchestrator") as mock_orchestrator,
            patch.object(cli_module, "setup_logging"),
            patch.object(cli_module, "exit") as mock_exit,
        ):
            mock_instance = Mock()
    ...
    def test_invalid_sample_size_handled_by_argparse(self):
        ...
        with (
            ...
            pytest.raises(SystemExit),  # argparse exits on invalid int
        ):
            studioeval.main()
    ...
    def test_missing_comm_config_file_handled(self):
        ...
        with (
            patch("sys.argv", ["studioeval.py", *test_args]),
            patch("core.evaluator.EvaluationOrchestrator"),
            patch.object(cli_module, "setup_logging"),
            patch.object(cli_module, "load_comm_config") as mock_load_config,
        ):
