...
from eval_datasets.huggingface.truthfulqa import (
    evaluate_tiny_truthfulqa,
    extract_mc1_fields,
    prepare_mc1_item,
)
...
class TestARCEvaluator:
    ...
    def test_arc_evaluator_handles_wrong_answers(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.arc.query_model")
    @patch("eval_datasets.huggingface.arc.time.sleep")
    def test_arc_evaluator_skips_malformed_items(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        mock_dataset = [
            ...
            },  # Valid
            {
                "question": "Test",
                "choices": "malformed",
            },  # Invalid choices - missing answerKey
            {"question": "Test"},  # Missing answerKey
            # NOTE: The current ARC evaluator doesn't handle missing 'question'
            # gracefully - it would crash. This is a potential improvement area
...
class TestCustomMCQEvaluator:
    ...
    @patch("eval_datasets.custom.custom_mcq.query_model")
    @patch("eval_datasets.custom.custom_mcq.time.sleep")
    def test_custom_mcq_evaluator_skips_invalid_items(
        self, mock_sleep, mock_query, mock_load
    ):
...
class TestMathEvaluator:
    ...
    def test_math_evaluator_basic(self, mock_query, mock_load):
        ...
        mock_dataset = [{"question": "What is 5 + 3?", "answer": 8}]
        mock_load.return_value = mock_dataset
    ...
    def test_math_evaluator_handles_float_answers(self, mock_query, mock_load):
        ...
        mock_dataset = [{"question": "What is 1/2 as decimal?", "answer": 0.5}]
        mock_load.return_value = mock_dataset
...
class TestGSM8KEvaluator:
    ...
    def test_gsm8k_evaluator_wrong_answer(self, mock_sleep, mock_query, mock_load):
        ...
        mock_dataset = [{"question": "Simple math problem", "answer": "5"}]
        mock_load.return_value = mock_dataset
        mock_query.return_value = (
            "Wrong calculation #### 7",
            {"tokens_per_second": 9.0},
...
class TestEvaluatorCommonPatterns:
    ...
    def test_evaluators_handle_custom_dataset_names(self):
        ...
        with (
            ...
            result = evaluate_grammar_dataset(
                "model", "test.jsonl", dataset_name="custom_grammar"
            )
    ...
    def test_evaluators_handle_sample_size_parameter(self):
        ...
        with patch(
            "eval_datasets.custom.custom_mcq.load_json_dataset_with_config"
        ) as mock_load:
    ...
    def test_evaluators_handle_seed_parameter(self):
        ...
        with patch(
            "eval_datasets.huggingface.arc.load_dataset_with_config"
        ) as mock_load:
            mock_load.return_value = []
    ...
    def test_extract_mc1_fields_standard_format(self):
        ...
        item = {
            ...
            "mc1_targets": {
                "choices": [
                    (
                        "Porcupines attack by running into predators, "
                        "impaling them with their quills."
                    ),
...
class TestTruthfulQAItemPreparation:
    ...
    def test_prepare_mc1_item_basic(self):
        ...
        item = {
            ...
            "mc1_targets": {"choices": ["Blue", "Green", "Red"], "labels": [1, 0, 0]},
        }
...
class TestTruthfulQAEvaluator:
    ...
    def test_truthfulqa_evaluator_basic(self, mock_sleep, mock_query, mock_load):
        ...
        # We need to determine the correct answers after shuffling. With
        # seed=42 and the current shuffling implementation, capture the actual
        # order so we know what letters the model should respond with.
    ...
    def test_truthfulqa_evaluator_wrong_answers(
        self, mock_prepare, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.truthfulqa.time.sleep")
    @patch("eval_datasets.huggingface.truthfulqa.prepare_mc1_item")
    def test_truthfulqa_evaluator_mixed_results(
        self, mock_prepare, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.truthfulqa.query_model")
    @patch("eval_datasets.huggingface.truthfulqa.time.sleep")
    def test_truthfulqa_evaluator_skips_malformed_items(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.truthfulqa.query_model")
    @patch("eval_datasets.huggingface.truthfulqa.time.sleep")
    def test_truthfulqa_evaluator_unparseable_model_response(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.truthfulqa.query_model")
    @patch("eval_datasets.huggingface.truthfulqa.time.sleep")
    def test_truthfulqa_evaluator_custom_parameters(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.truthfulqa.query_model")
    @patch("eval_datasets.huggingface.truthfulqa.time.sleep")
    def test_truthfulqa_evaluator_prompt_formatting(
        self, mock_sleep, mock_query, mock_load
    ):
...
class TestTruthfulQAIntegration:
    ...
    @patch("eval_datasets.huggingface.truthfulqa.query_model")
    @patch("eval_datasets.huggingface.truthfulqa.time.sleep")
    def test_truthfulqa_realistic_dataset_sample(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        mock_dataset = [
            {
                ...
                "mc2_targets": {
                    "choices": [
                        ...
                        (
                            "Porcupines attack by swinging their tails into "
                            "predators so that their quills make contact."
                        ),
                        (
                            "Porcupines may emit an invasive scent as a "
...
class TestCommonsenseQAEvaluator:
    ...
    def test_commonsense_qa_evaluator_basic(self, mock_sleep, mock_query, mock_load):
        ...
        result = evaluate_commonsense_qa(
            "test_model", dataset_name="test_commonsense_qa"
        )
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_evaluator_wrong_answers(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_evaluator_mixed_results(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_evaluator_skips_malformed_items(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_evaluator_unparseable_model_response(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_evaluator_custom_parameters(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_evaluator_prompt_formatting(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        assert (
            "A. sleep, B. bark, C. fly, D. dance, E. dance" in call_args
            or "A. sleep" in call_args
        )
    ...
    @patch("eval_datasets.huggingface.commonsense_qa.query_model")
    @patch("eval_datasets.huggingface.commonsense_qa.time.sleep")
    def test_commonsense_qa_uses_extract_letter_to_e(
        self, mock_sleep, mock_query, mock_load
    ):
        """Test that CommonsenseQA uses extract_letter_to_e (supports A-E)."""
        mock_dataset = [
            {
                "question": "Test question?",
                "choices": {
...
class TestLogiQAEvaluator:
    ...
    @patch("eval_datasets.huggingface.logiqa.query_model")
    @patch("eval_datasets.huggingface.logiqa.time.sleep")
    def test_logiqa_evaluator_skips_malformed_items(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.logiqa.query_model")
    @patch("eval_datasets.huggingface.logiqa.time.sleep")
    def test_logiqa_evaluator_unparseable_model_response(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        # Model gives unparseable response
        mock_query.return_value = (
            "This is complex logic problem requiring deep thinking.",
            {"tokens_per_second": 5.0},
        )
    ...
    @patch("eval_datasets.huggingface.logiqa.query_model")
    @patch("eval_datasets.huggingface.logiqa.time.sleep")
    def test_logiqa_evaluator_custom_parameters(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.logiqa.query_model")
    @patch("eval_datasets.huggingface.logiqa.time.sleep")
    def test_logiqa_evaluator_prompt_formatting(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        assert (
            "Read the following passage and answer the multiple-choice question."
            in call_args
        )
...
class TestHellaSwagEvaluator:
    ...
    @patch("eval_datasets.huggingface.hellaswag.query_model")
    @patch("eval_datasets.huggingface.hellaswag.time.sleep")
    def test_hellaswag_evaluator_skips_malformed_items(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.hellaswag.query_model")
    @patch("eval_datasets.huggingface.hellaswag.time.sleep")
    def test_hellaswag_evaluator_prompt_formatting(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.hellaswag.query_model")
    @patch("eval_datasets.huggingface.hellaswag.time.sleep")
    def test_hellaswag_evaluator_unparseable_response(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.hellaswag.query_model")
    @patch("eval_datasets.huggingface.hellaswag.time.sleep")
    def test_hellaswag_evaluator_custom_parameters(
        self, mock_sleep, mock_query, mock_load
    ):
...
class TestWinoGrandeEvaluator:
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_wrong_answers(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_mixed_results(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_skips_malformed_items(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_answer_format_handling(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_prompt_formatting(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        assert (
            "You are given a sentence with a blank indicated by an underscore"
            in call_args
        )
        assert "Choose the option (A or B) that best fills the blank" in call_args
        assert (
            "The trophy doesn't fit in the brown suitcase because _ is too small."
            in call_args
        )
        assert "A. the trophy" in call_args
        assert "B. the suitcase" in call_args
        assert (
            "Only respond with the letter (A or B) prefixed with 'Answer:'" in call_args
        )
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_unparseable_response(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        # Model gives unparseable response
        mock_query.return_value = (
            "I think both options are valid in this context",
            {"tokens_per_second": 5.0},
        )
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_evaluator_custom_parameters(
        self, mock_sleep, mock_query, mock_load
    ):
    ...
    @patch("eval_datasets.huggingface.winogrande.query_model")
    @patch("eval_datasets.huggingface.winogrande.time.sleep")
    def test_winogrande_uses_extract_mcq_letter(
        self, mock_sleep, mock_query, mock_load
    ):
        ...
        # Model response contains both A and B, but extract_mcq_letter should
        # handle this scenario correctly.
        mock_query.return_value = (
            "I think Answer: A is the best choice between A and B",
            {"tokens_per_second": 8.0},
        )
